...
class ZCL_IBMX_SERVICE definition
  public
  inheriting from ZCL_IBMX_SERVICE_ARCH
  create public .
...
public section.
  ...
  types:
    fieldname(30) type c .
  types BOOLEAN type ZIF_IBMX_SERVICE_ARCH~BOOLEAN .
  types CHAR type ZIF_IBMX_SERVICE_ARCH~CHAR .
  ...
  types:
    ...
    datetime(27) type c .
  types TT_STRING type ZIF_IBMX_SERVICE_ARCH~TT_STRING .
  types:
    tt_boolean type standard table of ZIF_IBMX_service_arch~boolean with non-unique default key .
  types:
    tt_char type standard table of ZIF_IBMX_service_arch~char with non-unique default key .
  types:
  ...
  types:
    tt_jsonobject type standard table of jsonobject with non-unique default key .
  types:
    t_array type standard table of jsonobject with non-unique default key .
  types:
  ...
  types TS_FORM_PART type ZIF_IBMX_SERVICE_ARCH~TS_FORM_PART .
  types TT_FORM_PART type ZIF_IBMX_SERVICE_ARCH~TT_FORM_PART .
  ...
  "! <p class="shorttext synchronized" lang="en">Returns a blank value.</p>
  "!
  ...
  class-methods BLANK_VALUE
    importing
      value(I_CHECK_ONLY) type BOOLEAN default C_BOOLEAN_FALSE
      !I_FIELD type ANY optional
    changing
      !C_FIELD type ANY optional
    returning
      value(E_IS_BLANK) type BOOLEAN .
  ...
  "! @raising ZCX_IBMX_SERVICE_EXCEPTION | Exception being raised.
  "!
  ...
  class-methods GET_FIELD_TYPE
    importing
      !I_FIELD type ANY
    exporting
      !E_TECHNICAL_TYPE type ZIF_IBMX_SERVICE_ARCH~CHAR
  ...
  class-methods GET_COMPONENTS
    ...
    exporting
      value(E_COMPONENTS) type ZIF_IBMX_SERVICE_ARCH~TT_STRING .
  ...
  class-methods GET_DATATYPE
    importing
      !I_FIELD type ANY
    returning
      value(E_DATATYPE) type ZIF_IBMX_SERVICE_ARCH~CHAR .
  ...
  class-methods ADD_HEADER_PARAMETER
    importing
      ...
      !I_IS_BOOLEAN type BOOLEAN default C_BOOLEAN_FALSE
    changing
      !C_HEADERS type ZIF_IBMX_SERVICE_ARCH~TS_REQUEST_PROP-HEADERS .
  "! <p class="shorttext synchronized" lang="en">Method for internal use.</p>
  "!
  ...
  methods CONSTRUCTOR
    importing
      ...
      !I_SSL_ID type ZIF_IBMX_SERVICE_ARCH~TY_SSL_ID optional
...
CLASS ZCL_IBMX_SERVICE IMPLEMENTATION.


* <SIGNATURE>---------------------------------------------------------------------------------------+
* | Static Public Method ZCL_IBMX_SERVICE=>ABAP_TO_JSON
* +-------------------------------------------------------------------------------------------------+
* | [--->] I_NAME                         TYPE        STRING(optional)
* | [--->] I_VALUE                        TYPE        ANY
* | [--->] I_DICTIONARY                   TYPE        ANY(optional)
  ...
  method abap_to_json.
    ...
    get_field_type(
      ...
      importing
        ...
        e_length         = lv_type_length ).
    if lv_type eq ZIF_IBMX_service_arch~c_datatype-dataref or lv_type eq ZIF_IBMX_service_arch~c_datatype-objectref.
    ...
    if lv_type ne ZIF_IBMX_service_arch~c_datatype-dataref and lv_type ne ZIF_IBMX_service_arch~c_datatype-objectref and
       lv_type ne ZIF_IBMX_service_arch~c_datatype-struct and lv_type ne ZIF_IBMX_service_arch~c_datatype-struct_deep and
       lv_type ne ZIF_IBMX_service_arch~c_datatype-itab.
      " simple type
      ...
      if lv_type eq 'C' or lv_type eq 'N' or lv_type eq 'g'.
        ...
        if lv_relative_type eq 'BOOLEAN' or
          ...
          if lv_str = c_blank(lv_type_length).
             lv_json = `null`.
          elseif lv_str eq c_boolean_true.
            lv_json = `true`.
          ...
          if ( lv_type_length = 0 and lv_str eq c_blank ) or ( lv_type_length > 0 and lv_str eq c_blank(lv_type_length) ).
    ...
    elseif lv_type eq ZIF_IBMX_service_arch~c_datatype-struct or lv_type eq ZIF_IBMX_service_arch~c_datatype-struct_deep.
      " structure

      lv_json = lv_json && `{`.
      clear lv_sep.

      lv_is_blank = blank_value(
        exporting
          i_check_only = c_boolean_true
          i_field = <i_value> ).

      if lv_is_blank eq c_boolean_false.
        get_components(
          exporting
            i_structure = <i_value>
          importing
    ...
    elseif lv_type eq ZIF_IBMX_service_arch~c_datatype-itab.
      " internal table
...
* <SIGNATURE>---------------------------------------------------------------------------------------+
* | Static Public Method ZCL_IBMX_SERVICE=>ADD_HEADER_PARAMETER
* +-------------------------------------------------------------------------------------------------+
* | [--->] I_PARAMETER                    TYPE        STRING
* | [--->] I_VALUE                        TYPE        STRING
* | [--->] I_IS_BOOLEAN                   TYPE        BOOLEAN (default =C_BOOLEAN_FALSE)
...
* +--------------------------------------------------------------------------------------</SIGNATURE>
  method add_header_parameter.
    ...
    data:
      ls_header type line of ZIF_IBMX_service_arch~ts_request_prop-headers,
...
* <SIGNATURE>---------------------------------------------------------------------------------------+
* | Static Public Method ZCL_IBMX_SERVICE=>ADD_QUERY_PARAMETER
* +-------------------------------------------------------------------------------------------------+
* | [--->] I_PARAMETER                    TYPE        STRING
* | [--->] I_VALUE                        TYPE        STRING
* | [--->] I_IS_BOOLEAN                   TYPE        BOOLEAN (default =C_BOOLEAN_FALSE)
...
* <SIGNATURE>---------------------------------------------------------------------------------------+
* | Instance Protected Method ZCL_IBMX_SERVICE->ADJUST_REQUEST_PROP
* +-------------------------------------------------------------------------------------------------+
* | [<-->] C_REQUEST_PROP                 TYPE        TS_REQUEST_PROP
* +--------------------------------------------------------------------------------------</SIGNATURE>
...
* <SIGNATURE>---------------------------------------------------------------------------------------+
* | Instance Public Method ZCL_IBMX_SERVICE->CHECK_HTTP_RESPONSE
* +-------------------------------------------------------------------------------------------------+
* | [--->] I_RESPONSE                     TYPE        TO_REST_RESPONSE(optional)
* | [--->] I_URL                          TYPE        TS_URL(optional)
...
* +--------------------------------------------------------------------------------------</SIGNATURE>
  method check_http_response.
    ...
    data:
      ...
      lv_strlen            type i,
      lv_code              type string,
      lo_service_exception type ref to ZCX_IBMX_service_exception.
    ...
    if lv_code ne '200' and   " ok
      ...
      try.
        ...
        catch ZCX_IBMX_service_exception into lo_service_exception.
...
* <SIGNATURE>---------------------------------------------------------------------------------------+
* | Instance Public Method ZCL_IBMX_SERVICE->CONSTRUCTOR
* +-------------------------------------------------------------------------------------------------+
* | [--->] I_URL                          TYPE        STRING(optional)
* | [--->] I_HOST                         TYPE        STRING(optional)
* | [--->] I_PROXY_HOST                   TYPE        STRING(optional)
...
* <SIGNATURE>---------------------------------------------------------------------------------------+
* | Instance Public Method ZCL_IBMX_SERVICE->GET_ACCESS_TOKEN
* +-------------------------------------------------------------------------------------------------+
* | [--->] I_REQUEST_PROP                 TYPE        TS_REQUEST_PROP(optional)
...
* <SIGNATURE>---------------------------------------------------------------------------------------+
* | Instance Public Method ZCL_IBMX_SERVICE->GET_APPNAME
* +-------------------------------------------------------------------------------------------------+
...
* <SIGNATURE>---------------------------------------------------------------------------------------+
* | Static Public Method ZCL_IBMX_SERVICE=>GET_COMPONENTS
* +-------------------------------------------------------------------------------------------------+
* | [--->] I_STRUCTURE                    TYPE        ANY
* | [<---] E_COMPONENTS                   TYPE        ZIF_IBMX_SERVICE_ARCH~TT_STRING
* +--------------------------------------------------------------------------------------</SIGNATURE>
...
* <SIGNATURE>---------------------------------------------------------------------------------------+
* | Static Public Method ZCL_IBMX_SERVICE=>GET_DATATYPE
* +-------------------------------------------------------------------------------------------------+
* | [--->] I_FIELD                        TYPE        ANY
* | [<-()] E_DATATYPE                     TYPE        ZIF_IBMX_SERVICE_ARCH~CHAR
* +--------------------------------------------------------------------------------------</SIGNATURE>
  method get_datatype.
...
* <SIGNATURE>---------------------------------------------------------------------------------------+
* | Static Public Method ZCL_IBMX_SERVICE=>GET_FIELD_TYPE
* +-------------------------------------------------------------------------------------------------+
* | [--->] I_FIELD                        TYPE        ANY
* | [<---] E_TECHNICAL_TYPE               TYPE        ZIF_IBMX_SERVICE_ARCH~CHAR
* | [<---] E_RELATIVE_TYPE                TYPE        STRING
...
* +--------------------------------------------------------------------------------------</SIGNATURE>
  method get_field_type.
    ...
    if e_relative_type is requested.
      if e_technical_type eq ZIF_IBMX_service_arch~c_datatype-objectref.
    ...
    if e_length is requested.
      if e_technical_type eq ZIF_IBMX_service_arch~c_datatype-c or
         e_technical_type eq ZIF_IBMX_service_arch~c_datatype-n.
        data(lo_elemdescr) = cast cl_abap_elemdescr( lo_typedescr ).
        if lo_elemdescr->output_length > 0.
          e_length = lo_elemdescr->output_length.
        else.
...
* <SIGNATURE>---------------------------------------------------------------------------------------+
* | Static Public Method ZCL_IBMX_SERVICE=>GET_FILE_EXTENSION
* +-------------------------------------------------------------------------------------------------+
...
* <SIGNATURE>---------------------------------------------------------------------------------------+
* | Static Public Method ZCL_IBMX_SERVICE=>GET_FULL_URL
* +-------------------------------------------------------------------------------------------------+
* | [--->] I_URL                          TYPE        TS_URL
* | [<-()] E_URL                          TYPE        STRING
* +--------------------------------------------------------------------------------------</SIGNATURE>
...
* <SIGNATURE>---------------------------------------------------------------------------------------+
* | Instance Public Method ZCL_IBMX_SERVICE->GET_REQUEST_PROP
* +-------------------------------------------------------------------------------------------------+
* | [--->] I_AUTH_METHOD                  TYPE        STRING (default =C_DEFAULT)
...
* <SIGNATURE>---------------------------------------------------------------------------------------+
* | Instance Public Method ZCL_IBMX_SERVICE->GET_REST_CLIENT
* +-------------------------------------------------------------------------------------------------+
* | [--->] I_REQUEST_PROP                 TYPE        TS_REQUEST_PROP
...
* +--------------------------------------------------------------------------------------</SIGNATURE>
  method get_rest_client.
    ...
    if not i_request_prop-header_content_type is initial.
      set_request_header( i_client = e_client i_name = ZIF_IBMX_service_arch~c_header_content_type i_value = ls_request_prop-header_content_type ).
    endif.

    " set 'Accept' header
    if not i_request_prop-header_accept is initial.
      set_request_header( i_client = e_client i_name = ZIF_IBMX_service_arch~c_header_accept i_value = ls_request_prop-header_accept ).
    endif.
...
* <SIGNATURE>---------------------------------------------------------------------------------------+
* | Instance Public Method ZCL_IBMX_SERVICE->HTTP_DELETE
* +-------------------------------------------------------------------------------------------------+
* | [--->] I_REQUEST_PROP                 TYPE        TS_REQUEST_PROP
...
* +--------------------------------------------------------------------------------------</SIGNATURE>
  method http_delete.
    ...
    e_response = execute( i_client = lo_client i_method = ZIF_IBMX_service_arch~c_method_delete ).
...
* <SIGNATURE>---------------------------------------------------------------------------------------+
* | Instance Public Method ZCL_IBMX_SERVICE->HTTP_GET
* +-------------------------------------------------------------------------------------------------+
* | [--->] I_REQUEST_PROP                 TYPE        TS_REQUEST_PROP
* | [<-()] E_RESPONSE                     TYPE        TO_REST_RESPONSE
* | [!CX!] ZCX_IBMX_SERVICE_EXCEPTION
* +--------------------------------------------------------------------------------------</SIGNATURE>
  method http_get.
    ...
    " execute GET request
    e_response = execute( i_client = lo_client i_method = ZIF_IBMX_service_arch~c_method_get ).
...
* <SIGNATURE>---------------------------------------------------------------------------------------+
* | Instance Public Method ZCL_IBMX_SERVICE->HTTP_PATCH
* +-------------------------------------------------------------------------------------------------+
* | [--->] I_REQUEST_PROP                 TYPE        TS_REQUEST_PROP
* | [<-()] E_RESPONSE                     TYPE        TO_REST_RESPONSE
* | [!CX!] ZCX_IBMX_SERVICE_EXCEPTION
* +--------------------------------------------------------------------------------------</SIGNATURE>
  method http_patch.
    ...
    e_response = execute( i_client = lo_client i_method = ZIF_IBMX_service_arch~c_method_patch ).
...
* <SIGNATURE>---------------------------------------------------------------------------------------+
* | Instance Public Method ZCL_IBMX_SERVICE->HTTP_POST
* +-------------------------------------------------------------------------------------------------+
* | [--->] I_REQUEST_PROP                 TYPE        TS_REQUEST_PROP
* | [<-()] E_RESPONSE                     TYPE        TO_REST_RESPONSE
* | [!CX!] ZCX_IBMX_SERVICE_EXCEPTION
* +--------------------------------------------------------------------------------------</SIGNATURE>
  method http_post.
    ...
    if i_request_prop-body is initial and i_request_prop-body_bin is initial.
      ...
      set_request_header( i_client = lo_client i_name = ZIF_IBMX_service_arch~c_header_content_type i_value = ZIF_IBMX_service_arch~c_mediatype-appl_json ).
    endif.

    " execute POST request
    e_response = execute( i_client = lo_client i_method = ZIF_IBMX_service_arch~c_method_post ).
...
* <SIGNATURE>---------------------------------------------------------------------------------------+
* | Instance Public Method ZCL_IBMX_SERVICE->HTTP_POST_MULTIPART
* +-------------------------------------------------------------------------------------------------+
* | [--->] I_REQUEST_PROP                 TYPE        TS_REQUEST_PROP
* | [--->] IT_FORM_PART                   TYPE        TT_FORM_PART
* | [<-()] E_RESPONSE                     TYPE        TO_REST_RESPONSE
...
* <SIGNATURE>---------------------------------------------------------------------------------------+
* | Instance Public Method ZCL_IBMX_SERVICE->HTTP_PUT
* +-------------------------------------------------------------------------------------------------+
* | [--->] I_REQUEST_PROP                 TYPE        TS_REQUEST_PROP
* | [<-()] E_RESPONSE                     TYPE        TO_REST_RESPONSE
* | [!CX!] ZCX_IBMX_SERVICE_EXCEPTION
* +--------------------------------------------------------------------------------------</SIGNATURE>
  method http_put.
    ...
    " execute PUT request
    e_response = execute( i_client = lo_client i_method = ZIF_IBMX_service_arch~c_method_put ).
...
* <SIGNATURE>---------------------------------------------------------------------------------------+
* | Static Public Method ZCL_IBMX_SERVICE=>BLANK_VALUE
* +-------------------------------------------------------------------------------------------------+
* | [--->] I_CHECK_ONLY                   TYPE        BOOLEAN (default =C_BOOLEAN_FALSE)
* | [--->] I_FIELD                        TYPE        ANY(optional)
* | [<-->] C_FIELD                        TYPE        ANY(optional)
* | [<-()] E_IS_BLANK                     TYPE        BOOLEAN
* +--------------------------------------------------------------------------------------</SIGNATURE>
method blank_value.

  data:
    lv_check_result  type boolean,
    lv_is_blank      type boolean value c_boolean_true,
    lv_type          type char,
    lv_type_length   type i,
    lt_compname      type tt_string,
    lv_relative_type type string,
  ...
  if i_check_only eq c_boolean_true and i_field is supplied.
    create data lr_data like i_field.
    assign lr_data->* to <c_field>.
    <c_field> = i_field.
  else.
    assign c_field to <c_field>.
  endif.
  ...
  if lv_type eq ZIF_IBMX_service_arch~c_datatype-dataref or lv_type eq ZIF_IBMX_service_arch~c_datatype-objectref.
    ...
    if <c_field> is bound.
      ...
      get_field_type(
        ...
        importing
          e_technical_type = lv_type
          e_relative_type  = lv_relative_type ).
      if lv_type eq ZIF_IBMX_service_arch~c_datatype-struct or lv_type eq ZIF_IBMX_service_arch~c_datatype-struct_deep.
        ...
        if lines( lt_compname ) = 1.
          read table lt_compname assigning <ls_compname> index 1.
          assign component <ls_compname> of structure <i_value> to <l_value>.
          if <ls_compname> eq '_' and <l_value> eq c_blank and lines( lt_compname ) = 1.
            " empty json object
            e_is_blank = c_boolean_true.
            return.
          endif.
        endif.
      endif.
    ...
    create data <c_field> like c_empty.
    assign <c_field>->* to <i_value>.
    <i_value> = c_empty.
  ...
  elseif lv_type eq ZIF_IBMX_service_arch~c_datatype-struct or lv_type eq ZIF_IBMX_service_arch~c_datatype-struct_deep.
    ...
    loop at lt_compname assigning <ls_compname>.
      ...
      lv_check_result = blank_value(
        exporting
          i_check_only = i_check_only
        changing
          c_field = <l_value> ).
      if lv_check_result eq c_boolean_false.
        lv_is_blank = c_boolean_false.
        if i_check_only eq c_boolean_true.
          exit.
        endif.
      endif.
  ...
  elseif lv_type eq ZIF_IBMX_service_arch~c_datatype-itab.
    ...
    assign <c_field> to <lt_value>.

    if lines( <lt_value> ) = 1.
      loop at <lt_value> assigning <l_value>. " loop (single step) since index operation is allowed
        lv_check_result = blank_value(
          exporting
            i_check_only = i_check_only
          changing
            c_field = <l_value> ).
        if lv_check_result eq c_boolean_true.
          e_is_blank = c_boolean_true.
          return.
        endif.
      endloop.
    ...
    if i_check_only eq c_boolean_false.
      clear <lt_value>.

      create data lr_data like line of <lt_value>.
      assign lr_data->* to <l_value>.
      blank_value( changing c_field = <l_value> ).
      insert <l_value> into table <lt_value>.
    ...
    if lv_type eq 'C' or lv_type eq 'N' or lv_type eq 'g'.
      lv_str = <i_value>.
      if not ( ( lv_type_length > 0 and lv_str eq c_blank(lv_type_length) ) or ( lv_type_length = 0 and lv_str eq c_blank ) ).
        lv_is_blank = c_boolean_false.
        if i_check_only eq c_boolean_false.
          <i_value> = c_blank.
        endif.
  ...
  e_is_blank = lv_is_blank.

endmethod.


* <SIGNATURE>---------------------------------------------------------------------------------------+
* | Static Protected Method ZCL_IBMX_SERVICE=>MERGE_STRUCTURE
* +-------------------------------------------------------------------------------------------------+
* | [--->] I_ALTERNATIVE                  TYPE        ANY
* | [<-->] C_BASE                         TYPE        ANY
* +--------------------------------------------------------------------------------------</SIGNATURE>
    ...
    loop at lt_compname assigning <lv_compname>.
      ...
      if lv_type eq ZIF_IBMX_service_arch~c_datatype-struct or lv_type eq ZIF_IBMX_service_arch~c_datatype-struct_deep.
        ...
        if <lv_comp_base> is initial.
          if lv_type eq ZIF_IBMX_service_arch~c_datatype-itab.
...
* <SIGNATURE>---------------------------------------------------------------------------------------+
* | Static Public Method ZCL_IBMX_SERVICE=>MOVE_DATA_REFERENCE_TO_ABAP
* +-------------------------------------------------------------------------------------------------+
* | [--->] I_DATA_REFERENCE               TYPE        DATA_REFERENCE
* | [<---] E_ABAP                         TYPE        ANY
* | [!CX!] ZCX_IBMX_SERVICE_EXCEPTION
...
* <SIGNATURE>---------------------------------------------------------------------------------------+
* | Static Public Method ZCL_IBMX_SERVICE=>NORMALIZE_URL
* +-------------------------------------------------------------------------------------------------+
* | [<-->] C_URL                          TYPE        TS_URL
* +--------------------------------------------------------------------------------------</SIGNATURE>
...
* <SIGNATURE>---------------------------------------------------------------------------------------+
* | Static Public Method ZCL_IBMX_SERVICE=>PARSE_JSON
* +-------------------------------------------------------------------------------------------------+
* | [--->] I_JSON                         TYPE        STRING
* | [--->] I_DICTIONARY                   TYPE        ANY(optional)
* | [<-->] C_ABAP                         TYPE        ANY
...
* +--------------------------------------------------------------------------------------</SIGNATURE>
  method parse_json.
    ...
    data:
      lv_json       type string,
      lt_dictionary type ZIF_IBMX_service_arch~tt_string,
    ...
    if not i_json is initial.
      ...
      try.
          lo_json->deserialize_int( exporting json = lv_json changing data = c_abap ).
            ...
            ZCL_IBMX_service=>raise_exception(
              exporting
                i_msgno = 20 ).
        endtry.
...
* <SIGNATURE>---------------------------------------------------------------------------------------+
* | Static Public Method ZCL_IBMX_SERVICE=>RAISE_EXCEPTION
* +-------------------------------------------------------------------------------------------------+
* | [--->] I_MSGNO                        TYPE        SY-MSGNO(optional)
* | [--->] I_MSGV1                        TYPE        STRING(optional)
* | [--->] I_MSGV2                        TYPE        STRING(optional)
...
* <SIGNATURE>---------------------------------------------------------------------------------------+
* | Instance Public Method ZCL_IBMX_SERVICE->SET_ACCESS_TOKEN
* +-------------------------------------------------------------------------------------------------+
* | [--->] I_ACCESS_TOKEN                 TYPE        TS_ACCESS_TOKEN
* +--------------------------------------------------------------------------------------</SIGNATURE>
...
* <SIGNATURE>---------------------------------------------------------------------------------------+
* | Instance Protected Method ZCL_IBMX_SERVICE->SET_DEFAULT_QUERY_PARAMETERS
* +-------------------------------------------------------------------------------------------------+
* | [<-->] C_URL                          TYPE        TS_URL
* +--------------------------------------------------------------------------------------</SIGNATURE>
...
* <SIGNATURE>---------------------------------------------------------------------------------------+
* | Instance Protected Method ZCL_IBMX_SERVICE->SET_URI_AND_AUTHORIZATION
* +-------------------------------------------------------------------------------------------------+
* | [--->] I_CLIENT                       TYPE        TS_CLIENT
* | [--->] I_REQUEST_PROP                 TYPE        TS_REQUEST_PROP
* | [<-()] E_URI                          TYPE        STRING
...
* <SIGNATURE>---------------------------------------------------------------------------------------+
* | Static Public Method ZCL_IBMX_SERVICE=>UNESCAPE_UNICODE
* +-------------------------------------------------------------------------------------------------+
* | [--->] I_IN                           TYPE        STRING
* | [<-()] E_OUT                          TYPE        STRING
* +--------------------------------------------------------------------------------------</SIGNATURE>
